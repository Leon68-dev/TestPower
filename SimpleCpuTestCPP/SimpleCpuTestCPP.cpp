...
public:
    ...
    void runIntegerTests()
    {
        ...
        r8.push_back(runMipsTest("Execute 8 bit Additions", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    sRes = s1 + s2;
            }));

        r8.push_back(runMipsTest("Execute 8 bit Substractions", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    sRes = s1 - s2;
            }));

        r8.push_back(runMipsTest("Execute 8 bit Multiplications", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    sRes = s1 * s2;
            }));

        r8.push_back(runMipsTest("Execute 8 bit Divisions", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    sRes = s1 / s2;
            }));
        ...
        r16.push_back(runMipsTest("Execute 16 bit Additions", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    hRes = h1 + h2;
            }));

        r16.push_back(runMipsTest("Execute 16 bit Substractions", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    hRes = h1 - h2;
            }));

        r16.push_back(runMipsTest("Execute 16 bit Multiplications", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    hRes = h1 * h2;
            }));

        r16.push_back(runMipsTest("Execute 16 bit Divisions", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    hRes = h1 / h2;
            }));
        ...
        r32.push_back(runMipsTest("Execute 32 bit Additions", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    iRes = i1 + i2;
            }));

        r32.push_back(runMipsTest("Execute 32 bit Substractions", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    iRes = i1 - i2;
            }));

        r32.push_back(runMipsTest("Execute 32 bit Multiplications", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    iRes = i1 * i2;
            }));

        r32.push_back(runMipsTest("Execute 32 bit Divisions", INT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < INT_OPERATIONS; i++)
                    iRes = i1 / i2;
            }));
    ...
    void runFloatingPointTests()
    {
        ...
        r32.push_back(runMipsTest("Execute 32 bit (Single) Additions", FLOAT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FLOAT_OPERATIONS; i++)
                    fRes = f1 + f2;
            }));

        r32.push_back(runMipsTest("Execute 32 bit (Single) Substractions", FLOAT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FLOAT_OPERATIONS; i++)
                    fRes = f1 - f2;
            }));

        r32.push_back(runMipsTest("Execute 32 bit (Single) Multiplications", FLOAT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FLOAT_OPERATIONS; i++)
                    fRes = f1 * f2;
            }));

        r32.push_back(runMipsTest("Execute 32 bit (Single) Divisions", FLOAT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FLOAT_OPERATIONS; i++)
                    fRes = f1 / f2;
            }));
        ...
        r64.push_back(runMipsTest("Execute 64 bit (Double) Additions", FLOAT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FLOAT_OPERATIONS; i++)
                    dRes = d1 + d2;
            }));

        r64.push_back(runMipsTest("Execute 64 bit (Double) Substractions", FLOAT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FLOAT_OPERATIONS; i++)
                    dRes = d1 - d2;
            }));

        r64.push_back(runMipsTest("Execute 64 bit (Double) Multiplications", FLOAT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FLOAT_OPERATIONS; i++)
                    dRes = d1 * d2;
            }));

        r64.push_back(runMipsTest("Execute 64 bit (Double) Divisions", FLOAT_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FLOAT_OPERATIONS; i++)
                    dRes = d1 / d2;
            }));
        ...
        r128.push_back(runMipsTest("Execute 128 bit (Extended) Additions", ldOps, [&]()
            {
                for (long long i = 0; i < ldOps; i++)
                    ldRes = ld1 + ld2;
            }));

        r128.push_back(runMipsTest("Execute 128 bit (Extended) Substractions", ldOps, [&]()
            {
                for (long long i = 0; i < ldOps; i++)
                    ldRes = ld1 - ld2;
            }));

        r128.push_back(runMipsTest("Execute 128 bit (Extended) Multiplications", ldOps, [&]()
            {
                for (long long i = 0; i < ldOps; i++)
                    ldRes = ld1 * ld2;
            }));

        r128.push_back(runMipsTest("Execute 128 bit (Extended) Divisions", ldOps, [&]()
            {
                for (long long i = 0; i < ldOps; i++)
                    ldRes = ld1 / ld2;
            }));
    ...
    void runFunctionTests()
    {
        ...
        results.push_back(runMipsTest("Test Square of 1234567890", FUNC_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FUNC_OPERATIONS; i++)
                    dummyRes = num * num;
            }));

        results.push_back(runMipsTest("Test Square root of 1234567890", FUNC_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FUNC_OPERATIONS; i++)
                    dummyRes = sqrt(num);
            }));
        ...
        results.clear();
        results.push_back(runMipsTest("Sinus", FUNC_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FUNC_OPERATIONS; i++)
                    dummyRes = sin(0.785);
            }));

        results.push_back(runMipsTest("Cosinus", FUNC_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FUNC_OPERATIONS; i++)
                    dummyRes = cos(0.785);
            }));

        results.push_back(runMipsTest("Tangent", FUNC_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FUNC_OPERATIONS; i++)
                    dummyRes = tan(0.785);
            }));

        results.push_back(runMipsTest("Arcus Tangent", FUNC_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FUNC_OPERATIONS; i++)
                    dummyRes = atan(0.785);
            }));
        ...
        results.clear();
        results.push_back(runMipsTest("Natural Logarithm", FUNC_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FUNC_OPERATIONS; i++)
                    dummyRes = log(100.0);
            }));

        results.push_back(runMipsTest("Exponent", FUNC_OPERATIONS, [&]()
            {
                for (long long i = 0; i < FUNC_OPERATIONS; i++)
                    dummyRes = exp(10.0);
            }));
    ...
    void runLogicalTests()
    {
        ...
        results.push_back(runMipsTest("Logical AND", LOGICAL_OPERATIONS, [&]()
            {
                for (long long i = 0; i < LOGICAL_OPERATIONS; i++)
                    vRes = v1 & v2;
            }));

        results.push_back(runMipsTest("Logical OR", LOGICAL_OPERATIONS, [&]()
            {
                for (long long i = 0; i < LOGICAL_OPERATIONS; i++)
                    vRes = v1 | v2;
            }));

        results.push_back(runMipsTest("Logical XOR", LOGICAL_OPERATIONS, [&]()
            {
                for (long long i = 0; i < LOGICAL_OPERATIONS; i++)
                    vRes = v1 ^ v2;
            }));

        results.push_back(runMipsTest("Logical NOT", LOGICAL_OPERATIONS, [&]()
            {
                for (long long i = 0; i < LOGICAL_OPERATIONS; i++)
                    vRes = ~v1;
            }));
